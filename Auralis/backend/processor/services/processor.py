...
QUERY_SPEC = ConsumerSpec(
    target_sample_rate=22050,
    target_channels=1,
    target_duration=None,  # whole clip, no windowing -- matches test.py's proven config.
    hop_duration=None,      # Revisit only once you're actually receiving long, continuous
    amplitude_range=(-1.0, 1.0),  # field recordings that windowing was designed for.
)
...
# Was 0.15 (an unvalidated placeholder) -- lowered to match test.py's
# threshold=0.0 (report everything) now that whole-clip processing removes
# the windowing-induced score degradation that made 0.15 too aggressive.
# Raise this once you've looked at real score distributions across enough
# samples to know where true positives cluster vs. false positives.
